package wedemo.view.timelineEditor;
...
public class NvsTimelineEditor extends RelativeLayout {
    ...
    public NvsTimelineTimeSpan addTimeSpan(long inPoint, long outPoint) {
        ...
        mTimelineTimeSpan.setTotalWidth(totalWidth);
        ...
        long cursorPositon = m_multiThumbnailSequenceView.mapTimelinePosFromX(m_screenCentral);
    ...
    public NvsTimelineTimeSpanExt addTimeSpanExt(long inPoint, long outPoint) {
        if (inPoint < 0 || outPoint < 0 || outPoint > m_timelineDuration || inPoint >= outPoint)
            return null;
        if (m_timeSpanType.compareTo("NvsTimelineTimeSpanExt") != 0)
            return null;
        NvsTimelineTimeSpanExt mTimelineTimeSpan = new NvsTimelineTimeSpanExt(m_context);
        mTimelineTimeSpan.setMultiThumbnailSequenceView(m_multiThumbnailSequenceView);
        mTimelineTimeSpan.setInPoint(inPoint);
        mTimelineTimeSpan.setOutPoint(outPoint);
        mTimelineTimeSpan.setPixelPerMicrosecond(m_pixelPerMicrosecond);
        int lLeftHandleWidth = mTimelineTimeSpan.getLLeftHandleWidth();
        ...
        int backgrdColor = Color.parseColor("#80ffff00");
        viewLeftMargin.setBackgroundColor(backgrdColor);
        int tmpMarginHeight = mTimelineTimeSpan.getLTopHandleHeight();
        ...
        mTimelineTimeSpan.setMarginChangeListener(new NvsTimelineTimeSpanExt.OnMarginChangeListener() {
            @Override
