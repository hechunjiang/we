...
public class NvAssetManager implements NvHttpRequest.NvHttpRequestListener, NvsAssetPackageManager.AssetPackageManagerCallback {
    ...
    private NvAssetManager(Context context) {
        ...
        if(streamingContext == null){
            NvsStreamingContext.init(context, Constants.SDK_LIC, NvsStreamingContext.STREAMING_CONTEXT_FLAG_SUPPORT_4K_EDIT);
            streamingContext = NvsStreamingContext.getInstance();
        }
...
//        initCustomStickerInfoFromSharedPreferences();
    ...
    private void searchAssetInBundlePath(int assetType, String dirPath) {
        ...
        String licSuffix = getAssetSuffix(NvAsset.ASSET_LIC);
        ...
        try {
            ...
            for (int index = 0; index < fileList.length; ++index) {
                ...
                if (filePath.endsWith(assetSuffix)) {
                    ...
                    if (assetType == NvAsset.ASSET_FILTER || assetType == NvAsset.ASSET_ANIMATED_STICKER || assetType == NvAsset.ASSET_FILTER_BEAUTY || assetType == NvAsset.ASSET_VIDEO_TRANSITION) {
                        for (int i = 0; i < fileList.length; ++i) {
                            //添加lic
                            String licPath = fileList[i];
                            if (TextUtils.isEmpty(licPath))
                                continue;
                            if (licPath.endsWith(licSuffix)) {
                                String[] fileTop = filePath.split("\\.");
                                String[] licTop = licPath.split("\\.");
                                if (fileTop[0].equals(licTop[0])) {
                                    String tmpPackagePath = "assets:/" + dirPath + File.separator + filePath;
                                    String licPackagePath = "assets:/" + dirPath + File.separator + licPath;
                                    NvAsset asset = installAssetPackage(tmpPackagePath, licPackagePath, assetType, true);
                                    ...
                                    //NvAsset assetInfo = findAsset(asset.uuid,assetType); //我们只查找当前的素材,不然我分割出来的文件夹没有

                                    //每次安装都清空了以前的，所以不需要检查重复,这样为了多文件夹管理
                                    assets.add(asset);
//                                    if (assetInfo == null) {
//                                        assets.add(asset);
//                                    } else {
...
//                                    }
                        ...
                        String tmpPackagePath = "assets:/" + dirPath + File.separator + filePath;
                        NvAsset asset = installAssetPackage(tmpPackagePath, null, assetType, true);
                        if (asset == null)
                            continue;
    ...
    private void searchAssetInLocalPath(int assetType, String dirPath) {
        ...
        if (dir.exists()) {
            ...
            for (File file : files) {
                ...
                NvAsset asset = installAssetPackage(filePath, null, assetType, false);
                if (asset == null)
                    continue;
    ...
    public String getAssetSuffix(int assetType) {
        String assetSuffix;
        switch (assetType) {
            ...
            case NvAsset.ASSET_FACE_BUNDLE_STICKER:
                assetSuffix = ".bundle";
                break;
            case NvAsset.ASSET_LIC:
                assetSuffix = ".lic";
                break;
            case NvAsset.ASSET_FILTER_BEAUTY:
                assetSuffix = ".videofx";
                break;
            default:
                assetSuffix = ".videofx";
    ...
    public NvAsset installAssetPackage(String filePath, String licPath, int assetType, boolean isReserved) {
        ...
        if (assetType == NvAsset.ASSET_FILTER_BEAUTY) {
            Log.e("weiwei", "1");
        }
            ...
            if (isSyncInstallAsset) {
                ...
                if (error == NvsAssetPackageManager.ASSET_PACKAGE_MANAGER_ERROR_NO_ERROR) {
                    ...
                    if (version > asset.version) {
                        error = manager.upgradeAssetPackage(filePath, licPath, asset.getPackageType(), false, packageId);
                        if (error == NvsAssetPackageManager.ASSET_PACKAGE_MANAGER_ERROR_NO_ERROR) {
                ...
                if (manager.getAssetPackageStatus(asset.uuid, asset.getPackageType()) == NvsAssetPackageManager.ASSET_PACKAGE_MANAGER_ERROR_ALREADY_INSTALLED) {
                    ...
                    if (assetInfo == null) {
                        ...
                        if (version > assetInfo.version) {
                            manager.upgradeAssetPackage(filePath, licPath, asset.getPackageType(), false, packageId);
                        }
                    }
                } else {
...
//        setCustomStickerInfoToSharedPreferences();
    ...
    private NvAsset findAsset(String uuid, int type) {
        ArrayList<NvAsset> assetList = assetDict.get(type);
        for (NvAsset asset : assetList) {
            if (asset.uuid.equals(uuid))
                return asset;
        }
    ...
    public void onDonwloadAssetSuccess(boolean success, String downloadPath, String downloadId) {
        ...
        NvAsset assetInfo = installAssetPackage(downloadPath, null, asset.assetType, false);
        if (isSyncInstallAsset) {
