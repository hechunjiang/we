...
public class ToastUtils {
    ...
    public static void showZDYToast(Context mContext, String addMessage, String message) {
        //使用布局加载器，将编写的toast_layout布局加载进来
        View view = LayoutInflater.from(mContext).inflate(R.layout.toast_layout, null);
        ((TextView) view.findViewById(R.id.tv_reward)).setText(addMessage);
        ((TextView) view.findViewById(R.id.tv_frequency)).setText(message);
        Toast toast = new Toast(mContext);
        toast.setGravity(Gravity.RIGHT | Gravity.BOTTOM, 30, 70);
        toast.setDuration(Toast.LENGTH_LONG);
        toast.setView(view);
        toast.show();
    }

    public static void showGoldCoinToast(Context mContext,String addMessage,String message){
        //使用布局加载器，将编写的toast_layout布局加载进来
        View view = LayoutInflater.from(mContext).inflate(R.layout.toast_gold_coin_layout, null);
        ((TextView)view.findViewById(R.id.tv_reward)).setText(addMessage);
        ((TextView)view.findViewById(R.id.tv_frequency)).setText(message);
        Toast toast=new Toast(mContext);
        toast.setGravity(Gravity.CENTER | Gravity.BOTTOM, 0, PhoneUtils.dp2px(mContext,110));
        toast.setDuration(Toast.LENGTH_LONG);
        toast.setView(view);
        toast.show();
    }

    public static void showToastConis(Context mContext, String title, String addMessage, int imgRes) {

        View view = LayoutInflater.from(mContext).inflate(R.layout.taost_coins, null);
        ImageView imageView = view.findViewById(R.id.iv_image);
        TextView textView = view.findViewById(R.id.tv_coinsnumber);
        TextView mTitle = view.findViewById(R.id.tv_discribtion);
        if (imgRes > 0) {
